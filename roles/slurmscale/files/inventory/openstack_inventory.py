...
def get_host_groups(inventory, refresh=False, cloud=None, private=False):
    ...
    if is_cache_stale(cache_file, cache_expiration_time, refresh=refresh):
        groups = to_json(get_host_groups_from_cloud(inventory, private=private))
...
def append_hostvars(hostvars, groups, key, server, namegroup=False, private=False):
    ip = server['interface_ip']
    if not ip and private and 'private_v4' in server:
...
def get_host_groups_from_cloud(inventory, private=False):
    ...
    for name, servers in firstpass.items():
        if len(servers) == 1 and use_hostnames:
            append_hostvars(hostvars, groups, name, servers[0], private=private)
        else:
            server_ids = set()
            ...
            if len(server_ids) == 1 and use_hostnames:
                ...
                for server in servers:
                    append_hostvars(
                        ...
                        namegroup=True, private=private)
...
def main():
    ...
    try:
        ...
        if args.list:
            output = get_host_groups(inventory, refresh=args.refresh, cloud=args.cloud, private=args.private)
        elif args.host:
